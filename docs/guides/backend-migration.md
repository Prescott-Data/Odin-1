...
Replace `OdinEngine(db)` with `OdinEngine(ArangoBackend(db))`:
...
result = engine.retrieve(seeds=["ExtractedEntities/your_entity"])
schema = inspect_schema(backend)
```

Connections and credentials remain caller-owned. `ArangoBackend` expects
`ExtractedEntities` and `ExtractedRelationships`; relationship edges use full
document IDs and a string `relationship` field. `EXTRACTED_FROM` provenance
edges are optional. See [Connecting ArangoDB](arangodb.md) for community mapping.

## Update imports and direct component calls

| Previous API | Current API |
| --- | --- |
| `OdinEngine(db)` | `OdinEngine(ArangoBackend(db))` |
...
Artifacts live in `OdinModels`, namespaced by database, graph collections,
community ID, and community mode. Old unscoped artifacts remain untouched and
are not reused. Plan for training on the first startup after migration.
